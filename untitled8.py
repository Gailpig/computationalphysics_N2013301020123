# -*- coding: utf-8 -*-
"""
Created on Sun Jun 19 22:50:59 2016
...
class CROMER_1(object):
    ...
    def calculate(self):
        for i in range(self.n):
            self.omg.append(self.omg[i]+(-self.g/self.l*math.sin(self.theta[i])-self.q[0]*self.omg[i]+self.F_D[2]*math.sin(2./3.*self.t[i]))*self.dt)
    ...
    def plot_theta1(self,_ax):
        _ax.axes.scatter(self.theta1, self.omg1, marker='x',color='m',label='q=0 F_D=1.2')
...
class CROMER_2(CROMER_1):
    ...
    def plot_theta2(self,_ax):
        _ax.axes.scatter(self.theta1,self.omg1,marker='o',color='b',label='q=0.5 F_D=1.2')

class CROMER_3(CROMER_1):
    def calculate(self):
        for i in range(self.n):
            self.omg.append(self.omg[i]+(-self.g/self.l*math.sin(self.theta[i])-self.q[2]*self.omg[i]+self.F_D[2]*math.sin(2./3.*self.t[i]))*self.dt)
            self.theta.append(self.theta[i]+self.omg[i+1]*self.dt)
            if self.theta[i+1]<-math.pi:
               self.theta[i+1]=self.theta[i+1]+2*math.pi
            if self.theta[i+1]>math.pi:
    ...
    def plot_theta3(self,_ax):
        _ax.axes.scatter(self.theta1,self.omg1,marker='o',color='b',label='q=1.2 F_D=1.2')
